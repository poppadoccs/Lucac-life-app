...
### 2026-04-23: Never use `git add -A` or `git add .` — sweeps in untracked work
- **WHAT WENT WRONG**: During S04 pre-flight commit, used `git add -A` to stage 5 modified source files. This swept in 70+ unrelated untracked files: Alex's fiverr freelance work (`fiverr_copypaste.html`, `fiverr_fill.py`), 50 thumbnail HTML/PNG files in `thumbnails/`, S03 draft scratch files (`S03_gemini_draft.md`, `S03_gpt_plan.md`), an entire `.gsd/` directory of planning artifacts, and an unfamiliar `docs/plans/S04-KIDS-GAME-PLAN-APPROVED.md` that hadn't been inspected. The single "pre-flight wiring" commit became an 81-file dump.
- **ROOT CAUSE**: `git add -A` does not distinguish between "files I just modified" and "files that happened to be untracked in the working tree." On a workspace with significant uncommitted other-work-in-progress, the safer-feeling shorthand is in fact a footgun. CLAUDE.md already cautions against this in the global rules but the warning didn't fire in the moment.
- **FIX**: `git reset HEAD~1` to undo the contaminated commit (keeps working tree intact), then `git add` each intended file by exact path, then re-commit. Untracked files returned to untracked state, no work lost.
- **RULE FOR NEXT TIME**: **Never run `git add -A` or `git add .`. Always `git add <specific paths>`.** When committing a logical chunk, list the files explicitly — even if it's tedious. If `git status` shows untracked files I didn't create, those are someone else's work-in-progress and must not be in my commit. The 30 seconds saved by `-A` is not worth the cleanup, the polluted history, or accidentally publishing private/draft work the user wasn't ready to commit.

### 2026-04-07: LLM arithmetic cannot be trusted — verify with JavaScript
- **WHAT WENT WRONG**: HomeworkHelper's AI tutor returned wrong arithmetic live on `lucac-life-app.vercel.app` during kid testing: `5+4=8`, `4+5+2=17`, and historically `5x10=40` (the original "5x10=40 incident"). Yana and Luca were seeing wrong answers in their homework helper. The existing `MATH_VERIFICATION_PROMPT` in `src/HomeworkHelper.jsx` was a prayer-based fix asking Groq to "double-check your arithmetic" — which LLMs ignore roughly 5% of the time.
- **ROOT CAUSE**: LLMs don't compute arithmetic, they pattern-match tokens. No amount of prompting ("please verify", "think step by step", "double-check before answering") actually makes them reliable computers. The 5x10=40 bug from the earlier build got a prompt-only fix that seemed to work for a while and then silently regressed because Groq drifted back to hallucinating numbers under certain input patterns. "We built a rule so it would never happen again" was technically true — we built a prompt rule — but the rule wasn't enforceable by prompt alone.
- **FIX**: Added `verifyMath(text)` helper in `src/HomeworkHelper.jsx` that post-processes every AI response before the student sees it. It regex-matches `<expression> = <stated answer>` patterns (handles `+`, `-`, `*`, `x`, `×`, `/`, `÷`), normalizes kid-friendly operators, whitelists only digits+operators+whitespace (so sandboxed `Function()` eval is safe against code injection), computes the true answer, and silently rewrites any wrong answer to the correct one while preserving the surrounding text. `doAICall` now uses `verifiedContent` everywhere (assistant message, frustration detection, celebration check). `MATH_VERIFICATION_PROMPT` was also updated to tell Groq its arithmetic will be auto-verified — this is an incentive-effect change, not a reliability change, but it reduces the number of silent corrections. Commit: `bf1c579`.
- **RULE FOR NEXT TIME**: **NEVER rely on LLM prompting to guarantee the correctness of anything that can be computed deterministically.** This applies to arithmetic, date math, string transformations, counting, unit conversions, sorting — anything where the right answer is computable. The pattern: ask the LLM to EXPLAIN, but have JavaScript compute and either inject or verify the ANSWER. Prompt-based "please double-check your work" instructions are hope, not code. For kid-facing features this is especially critical — kids trust the app and can't verify the math themselves. The app must not lie to them, and the only way to guarantee that is to make the truth come from code, not the LLM.
