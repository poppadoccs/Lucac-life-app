...
const groq = async (messages, system, maxTokens=300) => {
...
export default function LifeApp() {
  ...
  // Deep customization per profile
  ...
  const [fridgeChefResult, setFridgeChefResult] = useState(null);
  ...
  const fetchQuote = async () => {
    ...
    setQuoteLoad(true);
    try {
      const q = await groq([{role:"user",content:"Give one short powerful motivational quote for a single dad building a small business and raising two young kids. Under 20 words. Just the quote, no quote marks, no author."}], null, 80);
      setDailyQuote(q||"You are building something real. Every day counts.");
  ...
  // Per-profile customization helpers
  const myCustom = profileCustom[pid] || {};
  const saveCustom = (updates) => {
    const updated = {...profileCustom, [pid]:{...myCustom,...updates}};
    setProfileCustom(updated); store.set("profileCustom", updated);
  };
  const revertCustom = () => {
    const updated = {...profileCustom}; delete updated[pid];
    setProfileCustom(updated); store.set("profileCustom", updated);
  };
  const saveContacts = (updates) => {
    const updated = {...contactNumbers,...updates};
    setContactNumbers(updated); store.set("contactNumbers", updated);
  };

  // Per-profile font size and card size
  const fontSize = myCustom.fontSize || 1; // scale factor
  const hiddenTabs = myCustom.hiddenTabs || [];
  const tabNames = myCustom.tabNames || {};
  const profileBg = myCustom.bg || null;

  const askFridgeChef = async () => {
    if(!fridge.length){ alert("Add some items to your fridge first!"); return; }
    setFridgeChefLoad(true); setFridgeChefResult(null);
    try {
      const text = await groq([{role:"user",content:`I have these ingredients in my fridge/pantry: ${fridge.join(", ")}. 
Give me ONE detailed recipe I can make RIGHT NOW with only these ingredients (or a subset of them).
Include: recipe name, ingredients with amounts, step by step instructions, approximate nutrition facts (calories, protein, carbs, fat).
Be creative and make it sound delicious!`}], null, 600);
      setFridgeChefResult(text);
    } catch { setFridgeChefResult("Couldn't get a recipe right now — try again!"); }
  ...
  const sendAIChat = async (msg) => {
    ...
    try {
      ...
      const needsSearch = /weather|news|today|current|near me|price|score|latest|lawyer|doctor|restaurant|open|hours|address|phone/i.test(msg);
      let webContext = "";
      if(needsSearch && TAVILY_KEY) {
        webContext = await tavilySearch(msg);
      }
      const system = `You are the LUCAC Family App AI assistant. The user is ${current?.name}. Today is ${new Date().toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"})}. Kids: Yana (age 8) and Luca (age 6). Custody: Mom has Mon/Tue, Dad has Wed/Thu. Help with nutrition, meals, parenting, schedules, app features, motivation. Keep answers SHORT and friendly.${webContext?`\n\nCurrent web info:\n${webContext}`:""}`;
      const reply = await groq(newHistory.slice(-6), system, 400);
      setAiChat(h=>[...h,{role:"assistant",content:reply||"Sorry couldn't process that!"}]);
  ...
  const lookupNutrition = async (query) => {
    ...
    try {
      const text = await groq([{role:"user",content:`You are a nutrition database. The user typed: "${query}". Return ONLY valid JSON, no markdown: {"food":"string","serving":"string","calories":0,"protein":0,"carbs":0,"fat":0,"fiber":0,"sugar":0,"sodium":0,"note":"string"}. Use FDA standard values. If they mention a specific amount like "322 grams", calculate for that exact amount.`}], null, 300);
  ...
  const parseEmail = async()=>{
    ...
    try {
      const text = await groq([{role:"user",content:`Extract appointment info from this email. Return ONLY valid JSON, no markdown: {"title":"string","date":"YYYY-MM-DD","time":"HH:MM","note":"string"}. Email: ${aiEmail}`}], null, 200);
  ...
  const allAdultTabs = [{id:"home",icon:"🏠",label:"Home"},{id:"food",icon:"🍽️",label:"Food"},{id:"myfoods",icon:"🥩",label:"My Foods"},{id:"nutrition",icon:"🔥",label:"Nutrition"},{id:"goals",icon:"🎯",label:"Goals"},{id:"kids",icon:"⭐",label:"Kids"},{id:"calendar",icon:"📅",label:"Calendar"},{id:"routines",icon:"🔄",label:"Routines"},{id:"coparent",icon:"🤝",label:"Co-Parent"},
  ...
  const tabs = isKid
    ...
    : allAdultTabs.filter(t=>!hiddenTabs.includes(t.id)).map(t=>({...t,label:tabNames[t.id]||t.label}));

  const cardPad = {compact:"10px",normal:"16px",spacious:"22px",giant:"28px"}[myCustom.cardSize||"normal"] || "16px";
  const cardMB  = {compact:"8px", normal:"14px",spacious:"18px",giant:"24px"}[myCustom.cardSize||"normal"] || "14px";
  const fScale  = myCustom.fontSize||1;
  const activeBg = myCustom.bg || T.bg;

  const s = {
    wrap:{minHeight:"100vh",background:activeBg,color:T.text,fontFamily:"Georgia,serif",paddingBottom:"80px",fontSize:`${14*fScale}px`},
    ...
    page:{padding:"16px"},
    card:{background:T.card,border:`1px solid ${T.border}`,borderRadius:"16px",padding:cardPad,marginBottom:cardMB},
    h2:  {fontSize:`${17*fScale}px`,fontWeight:"bold",color:T.accent,margin:"0 0 12px 0"},
    h3:  {fontSize:`${11*fScale}px`,color:T.sub,marginBottom:"8px",textTransform:"uppercase",letterSpacing:"1px"},
    inp: {background:activeBg,border:`1px solid ${T.border}`,borderRadius:"10px",padding:"10px 14px",color:T.text,fontSize:`${14*fScale}px`,width:"100%",boxSizing:"border-box",fontFamily:"Georgia,serif"},
    txa: {background:activeBg,border:`1px solid ${T.border}`,borderRadius:"10px",padding:"10px 14px",color:T.text,fontSize:`${14*fScale}px`,width:"100%",boxSizing:"border-box",fontFamily:"Georgia,serif",minHeight:"80px",resize:"vertical"},
    btn: {background:T.accent,color:T.bg,border:"none",borderRadius:"10px",padding:"10px 18px",fontWeight:"bold",fontSize:`${14*fScale}px`,cursor:"pointer",fontFamily:"Georgia,serif"},
    btnSm:{background:T.accent,color:T.bg,border:"none",borderRadius:"8px",padding:"7px 12px",fontWeight:"bold",fontSize:`${12*fScale}px`,cursor:"pointer"},
    btnO:{background:"none",color:T.accent,border:`1px solid ${T.accent}`,borderRadius:"10px",padding:"8px 14px",fontSize:`${13*fScale}px`,cursor:"pointer"},
    ...
    tag: (a,c)=>({background:a?(c||T.accent):activeBg,color:a?T.bg:T.sub,border:`1px solid ${a?(c||T.accent):T.border}`,borderRadius:"20px",padding:"5px 11px",fontSize:`${12*fScale}px`,cursor:"pointer",margin:"3px",display:"inline-block"}),
    ...
    box: {background:activeBg,borderRadius:"12px",padding:"12px",textAlign:"center"},
  ...
  return (
    <div style={s.wrap}>
      ...
      {tab==="food"&&(<>
        ...
        {/* AI FRIDGE CHEF */}
        {fridge.length>0&&(<div style={{...s.card,borderLeft:`4px solid #f59e0b`}}>
          <div style={s.h2}>👨‍🍳 AI Fridge Chef</div>
          <div style={{color:T.sub,fontSize:"13px",marginBottom:"12px"}}>Using: {fridge.join(", ")}. Let AI create you a full detailed recipe right now!</div>
          <button style={{...s.btn,width:"100%",background:"linear-gradient(135deg,#f59e0b,#fb923c)"}} onClick={askFridgeChef} disabled={fridgeChefLoad}>
            {fridgeChefLoad?"👨‍🍳 Cooking up a recipe...":"🍳 Chef! What can I make right now?"}
          </button>
          {fridgeChefResult&&(<div style={{background:T.bg,borderRadius:"12px",padding:"14px",marginTop:"12px",whiteSpace:"pre-wrap",fontSize:"13px",lineHeight:1.8,border:`1px solid ${T.border}`}}>
            {fridgeChefResult}
            <button style={{...s.btnO,width:"100%",marginTop:"10px",fontSize:"12px"}} onClick={()=>setFridgeChefResult(null)}>Try Another Recipe ↻</button>
          </div>)}
        </div>)}

        {fridge.length>0&&matched.length===0&&!fridgeChefResult&&<div style={{...s.card,textAlign:"center",color:T.sub,padding:"24px"}}><div style={{fontSize:"32px"}}>🥄</div><div style={{marginTop:"8px"}}>No quick matches — tap AI Chef above for a custom recipe!</div></div>}
      ...
      {(tab==="kids"||tab==="rewards")&&(<>
        {/* CALL BUTTONS — kids see these big and friendly */}
        {isKid&&(<div style={{...s.card,textAlign:"center"}}>
          <div style={s.h2}>📞 Need to call someone?</div>
          <div style={{display:"flex",gap:"12px",marginTop:"10px"}}>
            {contactNumbers.dadPhone&&(<a href={`tel:${contactNumbers.dadPhone}`} style={{flex:1,background:"linear-gradient(135deg,#f59e0b,#fb923c)",borderRadius:"16px",padding:"20px 10px",textDecoration:"none",color:"#000",fontWeight:"bold",fontSize:"16px",display:"flex",flexDirection:"column",alignItems:"center",gap:"8px",boxShadow:"0 4px 20px #f59e0b44"}}>
              <span style={{fontSize:"36px"}}>👑</span>Call Dada
            </a>)}
            {contactNumbers.momPhone&&(<a href={`tel:${contactNumbers.momPhone}`} style={{flex:1,background:"linear-gradient(135deg,#a78bfa,#e040fb)",borderRadius:"16px",padding:"20px 10px",textDecoration:"none",color:"#fff",fontWeight:"bold",fontSize:"16px",display:"flex",flexDirection:"column",alignItems:"center",gap:"8px",boxShadow:"0 4px 20px #a78bfa44"}}>
              <span style={{fontSize:"36px"}}>💜</span>Call Mom
            </a>)}
            {!contactNumbers.dadPhone&&!contactNumbers.momPhone&&(<div style={{color:T.sub,fontSize:"13px",width:"100%",padding:"10px"}}>Ask a parent to add phone numbers in Settings → Customize 📱</div>)}
          </div>
        </div>)}
      ...
      {tab==="settings"&&isAdmin&&(<div style={s.card}>
        ...
        <div style={{display:"flex",gap:"6px",flexWrap:"wrap",marginBottom:"18px"}}>
          {[["profile","👤 Profile"],["customize","🎨 Customize"],["colors","🖌️ Colors"],["contacts","📞 Contacts"],["nutrition","🔥 Nutrition"],["alerts","🔔 Alerts"],["profiles","👨‍👩‍👧‍👦 Family"],["info","ℹ️ Info"]].map(([id,lbl])=>(
            <button key={id} style={{...s.btnO,background:subSet===id?T.accent:"none",color:subSet===id?T.bg:T.accent,fontSize:"11px",padding:"6px 10px"}} onClick={()=>setSubSet(id)}>{lbl}</button>
          ))}
        </div>

        {subSet==="profile"&&(()=>{
          const [dName,setDName]=useState(current?.name||"");
          const [dPin,setDPin]=useState("");
          const [saved,setSaved]=useState(false);
          const doSave=()=>{
            let updated=profiles;
            if(dName.trim()) updated=updated.map(p=>p.id===current.id?{...p,name:dName.trim()}:p);
            if(dPin.length>=4&&dPin.length<=6) updated=updated.map(p=>p.id==="admin"?{...p,pin:dPin}:p);
            saveProfiles(updated); setSaved(true); setTimeout(()=>setSaved(false),2000);
          };
          return(<div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
            <div style={s.h3}>Display Name</div>
            <input style={s.inp} value={dName} onChange={e=>setDName(e.target.value)} placeholder="Your name..."/>
            <div style={s.h3}>Change PIN (4–6 digits)</div>
            <input style={s.inp} type="password" maxLength={6} placeholder="New PIN..." value={dPin} onChange={e=>setDPin(e.target.value.replace(/\D/g,""))}/>
            <div style={{fontSize:"12px",color:T.sub}}>Current PIN length: {profiles.find(p=>p.id==="admin")?.pin?.length||4} digits</div>
            <button style={{...s.btn,background:saved?"#4ade80":T.accent,color:saved?"#000":T.bg}} onClick={doSave}>{saved?"✅ Saved!":"💾 Save Changes"}</button>
          </div>);
        })()}

        {subSet==="customize"&&(()=>{
          const ALL_TABS_LIST=[{id:"home",label:"Home"},{id:"food",label:"Food"},{id:"myfoods",label:"My Foods"},{id:"nutrition",label:"Nutrition"},{id:"goals",label:"Goals"},{id:"kids",label:"Kids"},{id:"calendar",label:"Calendar"},{id:"routines",label:"Routines"},{id:"coparent",label:"Co-Parent"},{id:"work",label:"Work"},{id:"settings",label:"Settings"}];
          const [localHidden,setLocalHidden]=useState(hiddenTabs);
          const [localNames,setLocalNames]=useState(tabNames);
          const [localFontSize,setLocalFontSize]=useState(myCustom.fontSize||1);
          const [localBg,setLocalBg]=useState(myCustom.bg||"");
          const [localCardSize,setLocalCardSize]=useState(myCustom.cardSize||"normal");
          const [saved,setSaved]=useState(false);
          const doSave=()=>{
            saveCustom({hiddenTabs:localHidden,tabNames:localNames,fontSize:localFontSize,bg:localBg||null,cardSize:localCardSize});
            setSaved(true); setTimeout(()=>setSaved(false),2000);
          };
          const doRevert=()=>{revertCustom();setLocalHidden([]);setLocalNames({});setLocalFontSize(1);setLocalBg("");setLocalCardSize("normal");};
          return(<div style={{display:"flex",flexDirection:"column",gap:"18px"}}>

            <div>
              <div style={s.h3}>🔤 Font Size</div>
              <div style={{display:"flex",gap:"8px",flexWrap:"wrap"}}>
                {[["Small",0.85],["Normal",1],["Large",1.15],["XL",1.3]].map(([lbl,val])=>(
                  <span key={val} style={{...s.tag(localFontSize===val),fontSize:"13px"}} onClick={()=>setLocalFontSize(val)}>{lbl}</span>
                ))}
              </div>
            </div>

            <div>
              <div style={s.h3}>📦 Card Size</div>
              <div style={{display:"flex",gap:"8px",flexWrap:"wrap"}}>
                {[["Compact","compact"],["Normal","normal"],["Spacious","spacious"],["Giant","giant"]].map(([lbl,val])=>(
                  <span key={val} style={{...s.tag(localCardSize===val),fontSize:"13px"}} onClick={()=>setLocalCardSize(val)}>{lbl}</span>
                ))}
              </div>
              <div style={{fontSize:"11px",color:T.sub,marginTop:"6px"}}>Giant = big boxes for things you use most 👆</div>
            </div>

            <div>
              <div style={s.h3}>🎨 Custom Background Color</div>
              <div style={{display:"flex",gap:"10px",alignItems:"center"}}>
                <input type="color" value={localBg||T.bg} onChange={e=>setLocalBg(e.target.value)} style={{width:"48px",height:"40px",borderRadius:"10px",border:"none",cursor:"pointer"}}/>
                <span style={{fontSize:"13px",color:T.sub}}>Override the theme background</span>
                {localBg&&<button style={{...s.btnO,fontSize:"11px",padding:"4px 8px"}} onClick={()=>setLocalBg("")}>Clear</button>}
              </div>
            </div>

            <div>
              <div style={s.h3}>👁️ Hide Tabs You Don't Use</div>
              <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
                {ALL_TABS_LIST.filter(t=>t.id!=="settings").map(t=>(
                  <div key={t.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",background:T.bg,borderRadius:"10px"}}>
                    <span style={{fontSize:"14px",color:localHidden.includes(t.id)?T.sub:T.text,textDecoration:localHidden.includes(t.id)?"line-through":"none"}}>{localNames[t.id]||t.label}</span>
                    <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
                      <span style={{fontSize:"11px",color:T.sub}}>Rename:</span>
                      <input style={{...s.inp,padding:"4px 8px",fontSize:"12px",width:"90px"}} placeholder={t.label} value={localNames[t.id]||""} onChange={e=>setLocalNames(n=>({...n,[t.id]:e.target.value}))}/>
                      <div onClick={()=>setLocalHidden(h=>h.includes(t.id)?h.filter(x=>x!==t.id):[...h,t.id])}
                        style={{width:"44px",height:"24px",borderRadius:"12px",background:localHidden.includes(t.id)?"#ef4444":"#4ade80",cursor:"pointer",position:"relative",transition:"all 0.2s"}}>
                        <div style={{position:"absolute",top:"2px",left:localHidden.includes(t.id)?"2px":"22px",width:"20px",height:"20px",borderRadius:"50%",background:"#fff",transition:"all 0.2s"}}/>
                      </div>
                    </div>
                  </div>
                ))}
              </div>
              <div style={{fontSize:"11px",color:T.sub,marginTop:"6px"}}>🟢 = visible · 🔴 = hidden. Settings tab always stays visible.</div>
            </div>

            <div style={{display:"flex",gap:"8px"}}>
              <button style={{...s.btn,flex:1,background:saved?"#4ade80":T.accent,color:saved?"#000":T.bg}} onClick={doSave}>{saved?"✅ Saved!":"💾 Save Customization"}</button>
              <button style={{...s.btnO,fontSize:"12px"}} onClick={doRevert}>↩ Revert All</button>
            </div>
          </div>);
        })()}

        {subSet==="contacts"&&(()=>{
          const [dDad,setDDad]=useState(contactNumbers.dadPhone||"");
          const [dMom,setDMom]=useState(contactNumbers.momPhone||"");
          const [saved,setSaved]=useState(false);
          return(<div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
            <div style={s.h3}>📞 Call Dada Button (kids see this)</div>
            <input style={s.inp} type="tel" placeholder="Dad's phone number..." value={dDad} onChange={e=>setDDad(e.target.value)}/>
            <div style={s.h3}>📞 Call Mom Button (kids see this)</div>
            <input style={s.inp} type="tel" placeholder="Mom's phone number..." value={dMom} onChange={e=>setDMom(e.target.value)}/>
            <div style={{background:T.bg,borderRadius:"10px",padding:"12px",fontSize:"12px",color:T.sub}}>💡 Kids will see big colorful buttons in the Stars tab to call these numbers instantly with one tap!</div>
            <button style={{...s.btn,background:saved?"#4ade80":T.accent,color:saved?"#000":T.bg}} onClick={()=>{saveContacts({dadPhone:dDad,momPhone:dMom});setSaved(true);setTimeout(()=>setSaved(false),2000);}}>{saved?"✅ Saved!":"💾 Save Numbers"}</button>
          </div>);
        })()}

        {subSet==="colors"&&(<div>
          <div style={s.h3}>Color code each person — their events, tasks & cards use their color</div>
        ...
        {subSet==="nutrition"&&(()=>{
          ...
          return(<div>
            ...
            <button style={{...s.btn,width:"100%",background:saved?"#4ade80":T.accent,color:saved?"#000":T.bg,marginTop:"8px"}} onClick={()=>{setSaved(true);setTimeout(()=>setSaved(false),2000);}}>{saved?"✅ Goals Saved!":"💾 Save Goals"}</button>
          </div>);
        })()}
        ...
        {subSet==="alerts"&&(<div style={{display:"flex",flexDirection:"column",gap:"14px"}}>
          ...
          <div style={{background:T.bg,borderRadius:"12px",padding:"14px",color:T.sub,fontSize:"13px",lineHeight:1.8}}>
            ...
            4. Come here and tap "Turn On Reminders"
          </div>
        </div>)}
        ...
        {subSet==="profiles"&&(()=>{
          ...
          return(<div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"8px",marginBottom:"18px"}}>
              ...
              <button style={s.btn} onClick={()=>{if(!newPName.trim())return;saveProfiles([...profiles,{id:`p_${Date.now()}`,name:newPName,emoji:newPEmoji||"😊",type:newPType}]);setNewPName("");setNewPEmoji("😊");setSaved(true);setTimeout(()=>setSaved(false),2000);}}>
        ...
        {subSet==="info"&&(<div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
          {[["📱","LUCAC Life v10.0 — The Deep Edition"],["⚡","Firebase real-time sync across all devices"],["🎨","Deep per-profile customization — ocean floor deep 😂"],["🌐","Groq AI + Tavily web search — real internet answers"],["👨‍🍳","AI Fridge Chef — custom recipes from your actual fridge"],["📞","One-tap kid call buttons for Dada & Mom"],["🔔","Browser reminders with custom timing"],["🎮","LUCAC LEGENDS — 5 worlds, 25 levels"],["👾","Danyells the Dark Queen 💅"],["💙","Built for Yana, Luca & Dada"],["🏢","Lucac LLC"]].map(([icon,v])=>(<div key={icon} style={{background:T.bg,borderRadius:"12px",padding:"12px",display:"flex",gap:"10px",alignItems:"center"}}><span style={{fontSize:"20px"}}>{icon}</span><div style={{fontSize:"14px"}}>{v}</div></div>))}
        </div>)}
      </div>)}
